...
export default function QuizDetailPage() {
  ...
  // Parse quiz configuration from description
  let quizConfig: any = {};
  ...
  try {
    ...
    if (parsed.config) {
      // New format: { description: "...", config: {...} }
      quizConfig = parsed.config;
      actualDescription = parsed.description || '';
    } else {
      // Legacy format: either old simple description or old config format
      if (typeof parsed === 'object' && (parsed.multiple_choice || parsed.gap_fill || parsed.true_false)) {
        // Old config format
        quizConfig = parsed;
        actualDescription = parsed.description || '';
      } else {
                ...
                <div className="text-center py-12">
                  ...
                  {quiz.quiz_type === 'gap_fill' ? (
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 mb-4">Add questions to make this quiz interactive.</p>
                      <Link href={`/dashboard/content/quizzes/${quiz.id}/questions/create`}>
                        <Button>
                          <Plus className="h-4 w-4 mr-2" />
                          Add First Question
