...
export function LessonWorkflowClient({ lessonId, userRole, userId }: LessonWorkflowClientProps) {
  ...
  const awardLessonPoints = async () => {
    try {
      ...
      // Collect all activities to award points for in a single batch
      ...
      // Send all activities in a single batch request (instead of N sequential calls)
      try {
        const response = await fetch('/api/student/award-points/batch', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (response.ok) {
          const data = await response.json();
          return Math.min(data.total_points_awarded || 0, maxPointsPerLesson);
        }
      } catch (error) {
        console.error('Error awarding lesson points (batch):', error);
      }
